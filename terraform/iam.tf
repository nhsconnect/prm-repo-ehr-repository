...
data "aws_iam_policy_document" "ehr_repo_access_log_bucket_policy" {
  ...
  statement {
    ...
    effect = "Allow"
    resources = ["${aws_s3_bucket.ehr_repo_access_logs.arn}/${local.ehr_repo_bucket_access_logs_prefix}*"]

    principals {
      identifiers = [aws_alb.alb-internal.arn]
      type        = "AWS"
    }
  }
...
resource "aws_iam_policy" "ehr_repo_access_logs_policy" {
  policy = data.aws_iam_policy_document.ehr_repo_access_log_bucket_policy.json
}
resource "aws_iam_role" "ehr_repo_access_logs_role" {
  name               = "${var.environment}-${var.component_name}-AcessLogRole"
  assume_role_policy = data.aws_iam_policy_document.ecs-assume-role-policy.json
  description        = "Role assumed by ${var.component_name} EHR Repo Accesslogs"
  tags = {
    Environment = var.environment
    CreatedBy = var.repo_name
  }
...
resource "aws_iam_role_policy_attachment" "ehr_repo_access_logs_role_policy_attachment" {
  role       = aws_iam_role.ehr_repo_access_logs_role.name
  policy_arn = aws_iam_policy.ehr_repo_access_logs_policy.arn
}